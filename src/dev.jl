...
# pois = fpm.pois
# apply!(x->normal_x(x,1),pois.z); BC!(pois.z)
# @inside pois.z[I] = ϕ[I]
# solver!(pois); BC!(fpm.ϕ)

# compute gradient of potential -> velocity
# for i ∈ 1:2
#     @WaterLily.loop ∇ϕ[I,i] -= pois.L[I,i]*WaterLily.∂(i,I,pois.x) over I ∈ inside(pois.x)
# end

# plot to check
# R = inside(fpm.ϕ)
# flood(fpm.ϕ[R],clims=(-radius,radius),levels=11,filled=true)
