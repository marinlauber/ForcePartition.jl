...
# plot results
plot(cumsum(@views(sim.flow.Δt[1:end-1]))./sim.L,force/2sim.L,label="pressure force",
     xlabel="tU/L",ylabel="2F/ρU²L",ylims=(0,2),xlims=(0,100),dpi=600)
plot!(cumsum(@views(sim.flow.Δt[1:end-1]))./sim.L,forcev/2sim.L,label="viscous force")
plot!(cumsum(@views(sim.flow.Δt[1:end-1]))./sim.L,fp/2sim.L,label="vorticity force (partition)")
plot!(cumsum(@views(sim.flow.Δt[1:end-1]))./sim.L,fa/2sim.L,label="added-mass force (partition)")
plot!(cumsum(@views(sim.flow.Δt[1:end-1]))./sim.L,-fv/2sim.L,label="viscous force (partition)")
# plot!(cumsum(@views(sim.flow.Δt[1:end-1]))./sim.L,(fp.+fa.-fv)/2sim.L,label="parition force")
